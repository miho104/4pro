...
function showDifficultyUI() {
  ...
  function pick(level) {
      ...
      console.log(rect.x+" "+rect.y+" "+rect.h+" "+rect.w);
      wrap.remove();
      gamestart();

  }
  btnEasy.addEventListener("click", () =>{ pick("easy");playVideo();});
  btnNormal.addEventListener("click", () => {pick("normal");playVideo();});
  btnHard.addEventListener("click", () => {pick("hard");playVideo();});
