...
// 与えた座標から主方向を求める（上下左右のいずれか）
function mainDirectionFromPoint(pt) {
    const c = screenCenter();
    const dx = pt.x - c.x;
    const dy = pt.y - c.y;
    // 角度ではなく軸距離でシンプルに判定：より絶対値が大きい軸を優先
...
// SVG <g> の塗り色を hsl/hex から別色へ滑らかに補間（単純なRGB線形）
...
function getFillOfGroup(g) {
    // 最初の子要素のfillを参照（circle/rect/polygonいずれか）
...
// 1ラウンド開始：ゾーン構築→対象ゾーン選定→popin or morph 実行→回答待ち
function startAhaRound() {
    ...
    // 念のため、空きゾーンを抽出
    const candidates = zoneSvgs.map((_, i) => i);
    const zoneIndex = randItem(candidates);
    const center = zoneCenter(zoneSvgs[zoneIndex].rect);
    ahaCorrectDir = mainDirectionFromPoint(center);

    const mode = AHA.chooseMode(); // "popin" | "colormorph"
    let morphCtrl = null;
    ...
    if (mode === "popin") {
        ...
        // colormorph: 既存shapeがなければ保険でpopin
...
function onAhaKeyDown(ev) {
    ...
    const speedComponent = Math.max(500, Math.round(9000 - clearMs)); // 速いほど高
...
function startMiniGame() {
    ...
    // 各ゾーンに初期図形をランダムに配置
    zoneSvgs.forEach(z => {
        const numShapes = randInt(1, 3); // 1〜3個の図形を配置
        for (let i = 0; i < numShapes; i++) {
            const type = randItem(SHAPES);
            const size = randItem(SIZES) * 0.8; // 少し小さめに
            const color = randItem(COLORS);
            const pad = size / 2 + 6;
            if (z.rect.w < 2 * pad || z.rect.h < 2 * pad) continue;
